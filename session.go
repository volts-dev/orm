...
func (self *TSession) txQuery(sql string, params ...interface{}) (res_dataset *dataset.TDataSet, res_err error) {
	...
	rows, res_err = self.tx.Query(sql, params...)
	if res_err != nil {
		return
	}
	// 无论如何都会返回一个Dataset
...
func (self *TSession) create(src interface{}, classic_create bool) (res_id interface{}, res_err error) {
	...
	for tbl, rel_vals := range lRefVals {
		...
		lNewVals[self.model.obj.GetRelationByName(tbl)] = record_id
	}
	...
	*/
	...
	// 被设置默认值的字段赋值给Val
	logger.Dbg("create GetDefault", self.model.obj.GetDefault())
	for k, v := range self.model.obj.GetDefault() {
		if lNewVals[k] == nil {
	...
	//1 lReturnKey := ""
	//logger.Dbg("TableByType", self.model.modelType, self.orm.TableByType(self.model.modelType))
	//lRecField := self.orm.TableByType(self.model.modelType).RecordField
	...
	if self.IsClassic || classic_create {
		...
		for _, name := range lNewTodo {
			lField := self.model.GetFieldByName(name)
...
func (self *TSession) separateValues(vals map[string]interface{}, mustFields map[string]bool, nullableFields map[string]bool, includeNil bool, mustPkey bool) (map[string]interface{}, map[string]map[string]interface{}, []string, error) {
	...
	// 保存关联表用于更新创建关联表数据
	for tbl, _ := range self.model.obj.GetRelations() {
		rel_vals[tbl] = make(map[string]interface{}) //NOTE 新建空Map以防Nil导致内存出错
		if val, has := vals[self.model.obj.GetRelationByName(tbl)]; has && val != nil {
	...
	for name, field := range self.model.GetFields() {
		...
		if val, has := vals[name]; has {
			...
			// TODO 优化锁
			// #相同名称的字段分配给对应表
			comm_tables := self.model.obj.GetCommonFieldByName(name) // 获得拥有该字段的所有表
			if comm_tables != nil {
				// 为各表预存值
...
func (self *TSession) write(src interface{}, context map[string]interface{}) (res_effect int64, res_err error) {
	...
	if self.IsClassic {
		...
		for field, _ := range lNewVals {
			var fobj IField
			fobj = self.model.GetFieldByName(field)
			if fobj == nil {
				lF := self.model.obj.GetRelatedFieldByName(field)
				if lF != nil {
					fobj = lF.RelateField
				}
			}
	...
	for tbl, ref_vals := range lRefVals {
		lFldName := self.model.obj.GetRelationByName(tbl)
...
func (self *TSession) read(classic_read bool) (*dataset.TDataSet, error) {
	...
	if len(self.Statement.Fields) > 0 {
		for name, allowed := range self.Statement.Fields {
			...
			fld := self.model.obj.GetFieldByName(name)
...
func (self *TSession) Delete(ids ...interface{}) (res_effect int64, err error) {
	...
	//#1 删除目标Model记录
	sql := fmt.Sprintf(`DELETE FROM %s WHERE %s in (%s); `, self.Statement.TableName(), id_field, idsToSqlHolder(ids...))
	...
	if len(less_ids) > 0 {
		...
		if self.IsClassic {
			...
			for _, name := range field_names {
				if f := self.model.obj.GetFieldByName(name); f != nil {
			...
			for _, fld := range fields {
				...
				if fld.Store() && fld.ColumnType() != "" && !(fld.IsRelatedField() && false) { //用false代替callable(field.base_field.column.translate)
...
func (self *TSession) struct_to_itfmap(src interface{}) (res_map map[string]interface{}) {
	...
	for i := 0; i < lType.NumField(); i++ {
		...
		// 字段必须在数据库里
		if fld := self.model.obj.GetFieldByName(lName); fld == nil {
			continue
		} else {
...
func (self *TSession) qualify(field IField, query *TQuery) string {
	res := self.Statement.inherits_join_calc(self.Statement.TableName(), field.Name(), query)
	/*
